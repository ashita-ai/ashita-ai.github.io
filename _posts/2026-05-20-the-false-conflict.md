...
The principle under it is narrow: a system can translate an *address* when it has a proven meaning-preserving map, but it cannot reconstruct an *effect whose value depends on content that moved* — and it must not pretend an internal address is still internal once it has crossed the system boundary.
