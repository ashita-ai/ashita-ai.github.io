...
Cigna used an algorithm called PXDX to screen claims for diagnosis-procedure mismatches. ProPublica found Cigna [denied 300,000 claims in two months](https://www.propublica.org/article/cigna-pxdx-medical-health-insurance-rejection-claims) using PxDx. Average processing time: 1.2 seconds per claim. Technically, patients could appeal. In practice, only about one in five did. Of those appealed, roughly [80% were overturned](https://www.pbs.org/newshour/show/how-algorithms-are-being-used-to-deny-health-insurance-claims-in-bulk), suggesting most denials were inappropriate. The system exploited the asymmetry between the cost of denying (near zero for Cigna) and the cost of appealing (substantial for a sick patient). For many patients, the denial was irreversible in practice.

UnitedHealthcare's subsidiary NaviHealth used an algorithm called nH Predict to manage nursing home stays. A class-action lawsuit [alleged a 90% error rate](https://www.statnews.com/2023/11/14/unitedhealth-class-action-lawsuit-algorithm-medicare-advantage/). STAT News found UnitedHealth pressured clinical employees to keep patient stays within 1% of the algorithm's projections. The prior authorization denial rate jumped from 10.9% in 2020 to 22.7% in 2022.
