...
[Mimir](https://github.com/Mimir-Review/mimir), an open-source bot reviewer being built by [Dan Getz](https://www.linkedin.com/in/dangetzjr/), takes bot review seriously: a confidence-tiered posting strategy with no inline cap; an explicit `IsApproximate()` signal that propagates from the semantic index to confidence ceilings, to prompt annotations the model itself reads, and to coverage disclosure on the summary comment; bounded fan-out with task isolation so a failed task does not cancel its siblings; a feedback loop via PR-comment reactions stored as labeled events. The work is largely his; we have been thinking through it together. If the design problem is interesting, reach out to him, or me.
