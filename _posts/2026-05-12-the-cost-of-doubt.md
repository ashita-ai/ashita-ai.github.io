...
[Mimir](https://github.com/Mimir-Review/mimir), an open-source bot reviewer [Dan Getz](https://www.linkedin.com/in/dangetzjr/) is building, is designed around this regime. Confidence-tiered posting with no inline cap. An `IsApproximate()` signal on the semantic index that propagates to confidence ceilings, to prompt annotations the model itself reads, and to coverage disclosure on the summary comment. Bounded fan-out with task isolation so a failed task does not cancel its siblings. A feedback loop via PR-comment reactions stored as labeled events. The design is largely his; we have been thinking through it together. If the design problem is interesting, reach out to him, or me.
