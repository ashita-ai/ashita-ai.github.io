...
In February 2024, Google paused Gemini's image generation feature after it produced historically inaccurate images: racially diverse depictions of groups like the Founding Fathers and Nazi German soldiers. Google could identify the symptom. It could not trace the output to its source — which training examples, which RLHF signal, which fine-tuning stage had produced the behavior. Without that chain, the fix was not targeted remediation. It was retraining and hope. The provenance failure did not just produce wrong outputs. It made wrong outputs impossible to debug systematically.
