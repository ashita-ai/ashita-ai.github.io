...
LIBOR (the London Interbank Offered Rate) underpinned approximately [over $300 trillion in financial instruments](https://www.newyorkfed.org/medialibrary/media/research/staff_reports/sr667.pdf). Between 2003 and 2012, banks systematically submitted false data. The UK Financial Services Authority documented at least 257 requests by Barclays traders to manipulate rate submissions across three benchmarks. LIBOR was eventually [phased out entirely](https://en.wikipedia.org/wiki/Libor_scandal), too compromised to reform. The ground truth was a coordinated fiction, and over $300 trillion in instruments were priced against it.
...
IBM spent approximately $4 billion acquiring healthcare data and analytics companies to build Watson Health. Watson for Oncology, the division's flagship product, was trained on synthetic cases from a small number of oncologists, not real patient data. It produced ["unsafe and incorrect" treatment recommendations](https://www.statnews.com/wp-content/uploads/2018/09/IBMs-Watson-recommended-unsafe-and-incorrect-cancer-treatments-STAT.pdf), including recommending a drug with a black box warning against the patient's exact condition. IBM sold the division, reportedly for approximately $1 billion. The ground truth the model learned from did not represent the reality it was deployed against.

Curtis Northcutt and colleagues [found](https://arxiv.org/abs/2103.14749) that at least 6% of ImageNet's validation labels are wrong. These are the benchmarks the field uses to rank models. The researchers found that lower-capacity models can outperform higher-capacity ones when test sets contain mislabeled data: the models ranking highest on standard benchmarks may simply be the ones that learned the label errors most precisely.
...
AI is moving from "assistant you might check" to "system that makes decisions." A New York lawyer [cited six fake cases](https://en.wikipedia.org/wiki/Mata_v._Avianca,_Inc.) generated by ChatGPT in a federal court filing. The cases did not exist. The citations were fabricated. The lawyer submitted them because the model presented them with the same confidence as real citations. The ground truth was never checked because the output looked exactly like ground truth.

Three things make the next few years worse.

First, the training data for the next generation of models is being poisoned by the current generation. Ilia Shumailov and colleagues [demonstrated](https://www.nature.com/articles/s41586-024-07566-y) in Nature that training on AI-generated data causes "irreversible defects" where "tails of the original content distribution disappear." As AI-generated content floods the internet, models increasingly train on their own outputs. The ground truth (human-generated data) is being diluted by approximations of itself. Shumailov warned: "The value of data collected about genuine human interactions with systems will be increasingly valuable in the presence of content generated by LLMs in data crawled from the Internet." Ground truth derived from human observation and failure, not generated by the model it governs, becomes the scarcest resource in the system.
...
The fix for Le Grand K was not a better cylinder. It was anchoring to something that does not drift. The infrastructure that survives the next paradigm shift will be built the same way.
